...
def _py(*args: str, password: Optional[str] = None,
    ...
    # Never pass the encryption password as a CLI argument — it would be
...
def direction_a(workdir: Path, password: str,
                arq_restore_bin: Optional[str]) -> Dict:
...
def direction_b(workdir: Path, plan_uuid: str, arq_dest: Path,
    ...
    r = _py("arq_reader.cli", "restore", str(arq_dest), folder, str(rdest),
            "--password-env", PW_ENV, "--verify-after", password=arq_pw)
...
def main(argv=None) -> int:
    ...
    add_common(pa)
    pa.add_argument("--password", default="compat-test")
    ...
    add_common(pall)
    pall.add_argument("--password", default="compat-test")
    ...
    if args.cmd == "direction-a":
        res = {"direction_a": direction_a(args.workdir, args.password,
                                          args.arq_restore_bin)}
