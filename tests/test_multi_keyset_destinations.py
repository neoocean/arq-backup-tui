...
class MultiKeysetDestinationTests(unittest.TestCase):
    ...
    def test_each_computer_has_independent_keyset(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            (password_first, res_a), (password_second, res_b) = results
            ...
            with self.assertRaises(CryptoError):
                decrypt_keyset(
                    ...
                    password_second,   # B's password against A's keyset
                )
    ...
    def test_restore_each_computer_independently(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            out_a = Path(td) / "out_a"
            out_a.mkdir()
            rs = Restore(str(dest), encryption_password=password_first)
            rs.restore(
            ...
            out_b = Path(td) / "out_b"
            out_b.mkdir()
            rs2 = Restore(str(dest), encryption_password=password_second)
            rs2.restore(
