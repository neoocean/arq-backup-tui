...
| **C** (cross-restore) | our writer → `arq_restore` (BSD reference) | ✅ **`diff -r` exit 0** + every file SHA-256 matches at Tree v3 (PR #56 / §4.3). Tree v4 is incompatible with the published `arq_restore` source which doesn't yet read the 38-byte trailing block; that's an `arq_restore` staleness, not a format gap on our side. |
