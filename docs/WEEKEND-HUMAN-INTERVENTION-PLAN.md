...
## ✅ 상태 (2026-05-24) — Arq 7 (Arq.app 7.44.1) 설치 후 종료

| 항목 | 상태 | 비고 |
|---|---|---|
| **P1 Strategy I** | ✅ **완료 (GREEN)** | Arq.app GUI가 우리 fresh-walk Tree v4를 byte-perfect 복원. production 버그 2개(backuprecord 경로 divisor 10⁵→10⁷, folder==planUUID 기본값) 발견·수정 (PR #184 / p4 CL 54128) |
| **P2 K4 first-walk-time** | ✅ **완료** | fresh-walk trailing block = 파일 btime (walk-time 아님) → create_time fallback 정당성 |
| **P6 keyset rotation (H1+H2)** | ✅ **완료** | 양방향 cross-tool 확인 + keyset_history 아카이브 parity 추가 (PR #185 / p4 CL 54140) |
| (V1 paired fingerprint) | ✅ **완료** | file/tree/chunk shape 일치; 차이는 backupplan.json config polymorphism뿐 |
| **P3 mixed Arq5/6/7** | 🚫 **범위 외** | Arq 5/6 미지원 결정 (2026-05-24) |
| **P5 binary-plist GUI 수용** | ⚪ **무의미** | 기본 emit이 JSON, P1이 JSON 경로 검증 |
| **P4 데몬 동시성 (A1/A2/A3)** | ⚠️ **미진행(결정)** | 우선순위 낮음 + 실 백업 데몬 방해 위험 |
| P7 clock jump / P8 network | — | GUI 아님 (VM/network 환경 필요) |

**양방향 byte-perfect interop 증명 완료** (우리 writer → Arq GUI / Arq → 우리 reader).
상세: `docs/ARQ7-GUI-INTEROP-2026-05-24.md`. 아래 P1–P8 본문은 원래 계획 그대로 보존.
